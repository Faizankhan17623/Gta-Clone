...
export function createGame({ hud, enemies, player, shooting, pickups }) {
  ...
  function waveClear(){if(state.betweenWaves||!state.alive||enemies.count)return;state.betweenWaves=true;missions?.record('waves');saveBest();setTimeout(()=>{if(state.alive){if(document.pointerLockElement)document.exitPointerLock();hud.showUpgrade(chooseUpgrade);}},1200);}
