...
export function createGame({ hud, enemies, player, shooting, pickups }) {
  ...
  let waveToken=0,missions=null,inventory=null,achievements=null;
  ...
  function onKill(headshot,type){addScore((type==='boss'?1000:100)+(headshot?50:0));missions?.record('kills');achievements?.record('kills');if(headshot){missions?.record('headshots');achievements?.record('headshots');}if(type==='boss'){hud.setBoss(0,1,false);missions?.record('bosses');achievements?.record('bosses');}if(Math.random()<.16)grantPowerup();}
  ...
  function waveClear(){if(state.betweenWaves||!state.alive||enemies.count)return;state.betweenWaves=true;missions?.record('waves');achievements?.record('waves');saveBest();setTimeout(()=>{if(state.alive){if(document.pointerLockElement)document.exitPointerLock();hud.showUpgrade(chooseUpgrade);}},1200);}
  ...
  return {state,start,update,restart,onKill,takeDamage,heal,togglePause,collect,addScore,spendScore,setMissions(v){missions=v;},setInventory(v){inventory=v;},setAchievements(v){achievements=v;},setDifficulty(v){state.difficulty=v;}};
}
