...
export function createGame({ hud, enemies, player, shooting, pickups }) {
  ...
  function onKill(headshot,type){const points=(type==='boss'?1000:100)+(headshot?50:0);state.score+=points;hud.setScore(state.score);if(type==='boss')hud.setBoss(0,1,false);if(Math.random()<.16)grantPowerup();}
  ...
  function waveClear(){if(state.betweenWaves||!state.alive||enemies.count)return;state.betweenWaves=true;saveBest();setTimeout(()=>{if(state.alive){if(document.pointerLockElement)document.exitPointerLock();hud.showUpgrade(chooseUpgrade);}},1200);}
  ...
  return {state,start,update,restart,onKill,takeDamage,togglePause,collect,setDifficulty(v){state.difficulty=v;}};
}
