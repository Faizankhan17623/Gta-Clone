...
export function createGame({ hud, enemies, player, shooting, pickups }) {
  ...
  function setMaxHealth(value){const ratio=state.health/state.maxHealth;state.maxHealth=value;state.health=Math.max(1,Math.round(value*ratio));hud.setHealth(state.health,state.maxHealth);}
  ...
  return {state,start,startWave,update,restart,onKill,takeDamage,heal,setMaxHealth,togglePause,collect,addScore,spendScore,setMissions(v){missions=v;},setInventory(v){inventory=v;},setAchievements(v){achievements=v;},setDaily(v){daily=v;},setDifficulty(v){state.difficulty=v;}};
}
