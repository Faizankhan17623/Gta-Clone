...
export function createGame({ hud, enemies, player, shooting, pickups }) {
  ...
  return {state,start,update,restart,onKill,takeDamage,heal,setMaxHealth,togglePause,collect,addScore,spendScore,setMissions(v){missions=v;},setInventory(v){inventory=v;},setAchievements(v){achievements=v;},setDaily(v){daily=v;},setDifficulty(v){state.difficulty=v;}};
}
