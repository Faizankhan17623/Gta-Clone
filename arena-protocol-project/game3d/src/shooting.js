...
export function createShooting({ camera, scene, weapon, effects, enemies, hud, onKill, onHit, sendShot }) {
  ...
  function tryFire(){
    ...
    for(let pellet=0;pellet<w.pellets;pellet++){
      ...
      if(oh.length&&(!point||oh[0].distance<eh[0].distance)){point=oh[0].point;target=null;barrel=oh[0].object.userData.explosive&&!oh[0].object.userData.spent?oh[0].object:null;const prop=oh[0].object;if(prop.userData.destructible&&!prop.userData.destroyed){prop.userData.health-=w.damage;if(prop.userData.health<=0){prop.userData.destroyed=true;prop.visible=false;hud.toast('COVER DESTROYED','good');}}}
